...
const Presentation = () => {
  ...
  useEffect(() => {
    const fetchDrawings = async () => {
      if (!selectedProjectId) return;
      try {
        const response = await axios.get(`http://localhost:5000/presentation/files/proj/${selectedProjectId}`);
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        `http://localhost:5000/presentation/upload/${selectedProjectId}`,
        formData,
        {
      ...
      const fetchDrawings = async () => {
        try {
          const res = await axios.get(`http://localhost:5000/presentation/files/proj/${selectedProjectId}`);
  ...
  const downloadDwg = async (id) => {
    try {
      const response = await axios.get(`http://localhost:5000/presentation/download/${id}`, {
        responseType: 'blob',
      });
  ...
  const viewDrawing = (id) => {
    window.open(`http://localhost:5000/presentation/view/${id}`, '_blank');
  };

  // Mark file as final
  const markAsFinal = async (id) => {
    try {
      const response = await axios.post(`http://localhost:5000/presentation/Finalized/${id}`);
  ...
  const markAsNotFinal = async (id) => {
    try {
      const response = await axios.post(`http://localhost:5000/presentation/Not-Finalized/${id}`);
