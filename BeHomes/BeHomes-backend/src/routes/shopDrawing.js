...
const Pdf = require('../models/ShopDrawing');
...

{
        ...
        try {
            ...
            const [execution, project] = await Promise.all([
                ExecutionPlanning.findOneAndUpdate(
                    ...
                    { $set: { "Shop_Drawing.Final_Date": date } },
                    { new: true }
                ),
            ...
            if (execution && execution.Shop_Drawing.Execution_date) {
                await updateProjectStatus.updateProjectStatus(
                    file.ProjectId,
                    execution.Shop_Drawing.Execution_date,
                    execution.Shop_Drawing.Final_Date
                );
            }

            // Move project to the "ShopDrawing" stage
            await Project.findByIdAndUpdate(file.ProjectId, {
                currentStage: "Shop_Drawing",
                $push: { "addedStages": "Shop_Drawing" },
                new: true
            });
    ...
    router.post("/Not-Finalized/:id", async (req, res) => {
        try {
            ...
            if ("Shop_Drawing" !== project.currentStage) {
            ...
            await Promise.all([
                ExecutionPlanning.findOneAndUpdate(
                    { ProjectId: file.ProjectId },
                    { $set: { "Shop_Drawing.Final_Date": null } }
                ),
                Project.findByIdAndUpdate(file.ProjectId, {
                    $pull: { "addedStages": "Shop_Drawing" },
