...
const Pdf = require('../models/Dispatch');
...
router.get('/files/:id', async (req, res) => {
    ...
    try {
        ...
        const pdfs = await Pdf.find(query, '-data -mainFile.data -challan.data -invoice.data -lights.data -accessories.data  -challan.data -accessories.data');
        res.status(200).json(pdfs);
    } catch (error) {
        res.status(500).json({ message: 'Error fetching PDFs', error: error.message });
    }
...
router.get('/files/proj/:id', async (req, res) => {
    ...
    try {
        ...
        const files = await Pdf.find(query, '-data -mainFile.data -challan.data -invoice.data ').exec();
        res.status(200).json({
            files: files,
            message: 'Success'
        });
...
router.post("/Finalized/:id", async (req, res) => {
    try {
        ...
        if (execution && execution.Dispatch.Execution_date) {
            await updateProjectStatus.updateProjectStatus(
                file.ProjectId,
                execution.Dispatch.Execution_date,
                execution.Dispatch.Final_Date
            );
        }

        // Move project to the "Dispatch" stage
        await Project.findByIdAndUpdate(file.ProjectId, {
            currentStage: "Dispatch",
            $push: { "addedStages": "Dispatch" },
            new: true
        });
...
router.post("/Not-Finalized/:id", async (req, res) => {
    try {
        ...
        if ("Dispatch" !== project.currentStage) {
        ...
        // Revert Dispatch final date and reset project stage
        await Promise.all([
            ExecutionPlanning.findOneAndUpdate(
                { ProjectId: file.ProjectId },
                { $set: { "Dispatch.Final_Date": null } }
            ),
            Project.findByIdAndUpdate(file.ProjectId, {
                $pull: { "addedStages": "Dispatch" },
