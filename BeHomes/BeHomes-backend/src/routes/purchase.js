...
const Pdf = require('../models/Purchase');
...
router.get('/files/proj/:id', async (req, res) => {
    ...
    try {
        ...
        const files = await Pdf.find(query, '-data -mainFile.data -hw.data -wooden.data -lights.data -accessories.data').exec();
        res.status(200).json({
            files: files,
            message: 'Success'
        });
...
router.post("/Finalized/:id", async (req, res) => {
    try {
        ...
        if (execution && execution.Purchase.Execution_date) {
            await updateProjectStatus.updateProjectStatus(
                file.ProjectId,
                execution.Purchase.Execution_date,
                execution.Purchase.Final_Date
            );
        }

        // Move project to the "Purchase" stage
        await Project.findByIdAndUpdate(file.ProjectId, {
            currentStage: "Purchase",
            $push: { "addedStages": "Purchase" },
            new: true
        });
...
router.post("/Not-Finalized/:id", async (req, res) => {
    try {
        ...
        if ("Purchase" !== project.currentStage) {
        ...
        await Promise.all([
            ExecutionPlanning.findOneAndUpdate(
                { ProjectId: file.ProjectId },
                { $set: { "Purchase.Final_Date": null } }
            ),
            Project.findByIdAndUpdate(file.ProjectId, {
                $pull: { "addedStages": "Purchase" },
