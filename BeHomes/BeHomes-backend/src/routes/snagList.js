...
const Pdf = require('../models/SnagList');
...
router.post("/Finalized/:id", async (req, res) => {
    try {
        ...
        if (execution && execution.Snag_List.Execution_date) {
            await updateProjectStatus.updateProjectStatus(
                file.ProjectId,
                execution.Snag_List.Execution_date,
                execution.Snag_List.Final_Date
            );
        }

        // Move project to the "SnagList" stage
        await Project.findByIdAndUpdate(file.ProjectId, {
            currentStage: "Snag_List",
            $push: { "addedStages": "Snag_List" },
            new: true
        });

        console.log(`Project ${file.ProjectId} moved to SnagList stage`);
...
router.post("/Not-Finalized/:id", async (req, res) => {
    try {
        ...
        if ("Snag_List" !== project.currentStage) {
        ...
        await Promise.all([
            ExecutionPlanning.findOneAndUpdate(
                { ProjectId: file.ProjectId },
                { $set: { "Snag_List.Final_Date": null } }
            ),
            Project.findByIdAndUpdate(file.ProjectId, {
                $pull: { "addedStages": "Snag_List" },
                currentStage: CurrentStage,
                new: true
            })
        ]);

        console.log(`Project ${file.ProjectId} reverted to previous stage: ${project.previousStage}`);
